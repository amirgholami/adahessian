...
from fairseq.modules import LayerNorm, MultiheadAttention, NormSelect, DropoutSelect
...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args):
        ...
        self.self_attn_layer_norm = NormSelect(args.encoder_norm_self, self.embed_dim, args.encoder_attention_heads)
        ...
        self.final_layer_norm = NormSelect(args.encoder_norm_ff, self.embed_dim, args.encoder_attention_heads)
    ...
    def forward(self, x, encoder_padding_mask, attn_mask=None):
        ...
        x = self.dp( x, self.dropout, self.training )
        x = residual + x
        ...
        x = self.dp( x, self.activation_dropout, self.training )
        x = self.fc2(x)
        # x = F.dropout(x, p=self.dropout, training=self.training)
        x = self.dp( x, self.dropout, self.training )
...
class TransformerDecoderLayer(nn.Module):
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False):
        ...
        self.self_attn_layer_norm = NormSelect(args.decoder_norm_self, self.embed_dim, args.decoder_attention_heads)
        ...
        if no_encoder_attn:
            ...
            self.encoder_attn_layer_norm = NormSelect(args.decoder_norm_self, self.embed_dim, args.decoder_attention_heads)

        self.fc1 = Linear(self.embed_dim, args.decoder_ffn_embed_dim)
        self.fc2 = Linear(args.decoder_ffn_embed_dim, self.embed_dim)

        # self.final_layer_norm = LayerNorm(self.embed_dim, export=export)
        self.final_layer_norm = NormSelect(args.decoder_norm_ff, self.embed_dim, args.decoder_attention_heads)
