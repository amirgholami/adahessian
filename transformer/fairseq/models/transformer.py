...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        if args.encoder_normalize_before:
            ...
            self.layer_norm = NormSelect(args.encoder_norm_ff, embed_dim, args.encoder_attention_heads)
        else:
    ...
    def forward(self, src_tokens, src_lengths, cls_input=None, return_all_hiddens=False):
        ...
        if self.layer_norm:
            x = self.layer_norm(x, pad_mask=encoder_padding_mask)
            if return_all_hiddens:
                encoder_states[-1] = x
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        if args.decoder_normalize_before and not getattr(args, 'no_decoder_final_norm', False):
            # self.layer_norm = LayerNorm(embed_dim)
            self.layer_norm = NormSelect(args.decoder_norm_ff, embed_dim, args.decoder_attention_heads)
        else:
            self.layer_norm = None
        self.dp = DropoutSelect( args.dropout_type, args.dropout_gama )
    ...
    def extract_features(
        ...
        **unused,
        ...
        if self.layer_norm:
            x = self.layer_norm(x, pad_mask=self_attn_padding_mask)
        ...
        return x, {'attn': attn, 'inner_states': inner_states, \
                   'encoder_states': encoder_out['encoder_states'] if encoder_out is not None else None}
...
@register_model_architecture('transformer', 'transformer_iwslt_de_en_v2_ms')
def transformer_iwslt_de_en_v2_ms(args):
