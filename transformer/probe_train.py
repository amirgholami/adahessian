...
def main(args, init_distributed=False):
    ...
    eigenvalues, eigenvectors = train_hessian(args, trainer, task, epoch_itr, sample_iter=10, top_n=10)
    res_dict = {
        "eigenvalues": eigenvalues,
        "eigenvectors": eigenvectors,
    }
    print(eigenvalues)
    pk.dump(res_dict, open(f"iwslt_result_hessian_analysis/{args.optimizer}_epoch100.pkl", "wb"))
    exit()
...
def train_hessian(args, trainer, task, epoch_itr, sample_iter=1, maxIter=500, tol=1e-4, top_n=1, ignore_grad=False):
    ...
    while computed_dim < top_n:
        ...
        for i in range(maxIter):
            ...
            loss, sample_size, logging_output, gradsH, tmp_eigenvalue, Hv = trainer.task.train_step_hessian(
                         samples_hessian, trainer.model, trainer.criterion, trainer.optimizer,
                         ignore_grad, v=v)
            v = normalization(Hv)
            if eigenvalue == None:
